...
public class BathEntity extends PlaneEntity implements IEntityRaytraceable
{
    ...
    public void updateVehicle()
    {
        if(this.isFlying() && this.getControllingPassenger() != null)
        {
            for(int i = 0; i < 4; i++)
            {
                this.world.addParticle(ParticleTypes.DRIPPING_WATER, this.posX - 0.25 + 0.5 * rand.nextGaussian(), this.posY + 0.5 * rand.nextGaussian(), this.posZ - 0.25 + 0.5 * rand.nextGaussian(), 0, 0, 0);
            }
        }
    }
