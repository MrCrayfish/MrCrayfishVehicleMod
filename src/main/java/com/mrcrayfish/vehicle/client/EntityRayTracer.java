...
public class EntityRayTracer
{
    ...
    private static void createBodyTransforms(List<MatrixTransformation> transforms, EntityType<? extends VehicleEntity> entityType)
    {
        ...
        transforms.add(MatrixTransformation.createRotation(Vector3f.XP, (float) bodyPosition.getRotX()));
        transforms.add(MatrixTransformation.createRotation(Vector3f.YP, (float) bodyPosition.getRotY()));
        transforms.add(MatrixTransformation.createRotation(Vector3f.ZP, (float) bodyPosition.getRotZ()));
    ...
    public static void createPartTransforms(ISpecialModel model, PartPosition partPosition, HashMap<RayTracePart, List<MatrixTransformation>> parts, List<MatrixTransformation> transformsGlobal)
    {
        ...
        transforms.add(MatrixTransformation.createTranslation(0.0F, 0.5F, 0.0F));
        transforms.add(MatrixTransformation.createRotation(Vector3f.XP, (float) partPosition.getRotX()));
        transforms.add(MatrixTransformation.createRotation(Vector3f.YP, (float) partPosition.getRotY()));
        transforms.add(MatrixTransformation.createRotation(Vector3f.ZP, (float) partPosition.getRotZ()));
    ...
    public static void createPartTransforms(double x, double y, double z, Vector3f rotation, double scale, List<MatrixTransformation> transforms)
    {
        ...
        if (rotation.getX() != 0)
        {
            transforms.add(MatrixTransformation.createRotation(Vector3f.XP, rotation.getX()));
        }
        if (rotation.getY() != 0)
        {
            transforms.add(MatrixTransformation.createRotation(Vector3f.YP, rotation.getY()));
        }
        if (rotation.getZ() != 0)
        {
            transforms.add(MatrixTransformation.createRotation(Vector3f.ZP, rotation.getY()));
        }
    }
    ...
    /**
     * Version of {@link EntityRayTracer#createFuelPartTransforms createFuelPartTransforms} that sets the Vector3f of rotation to Y
     *
     ...
     * @param rotation         part's rotation yaw (Y Vector3f)
     * @param scale            part's scale
     * @param parts            map of all parts to their transforms
     * @param transformsGlobal transforms that construct to all parts for this entity
     */
    ...
    public static class MatrixTransformation
    {
        ...
        public static MatrixTransformation createRotation(Vector3f Vector3f, float angle)
        {
            return new MatrixTransformation(MatrixTransformationType.ROTATION, Vector3f.getX(), Vector3f.getY(), Vector3f.getZ(), angle);
        }
    ...
    /**
     * Rotates the x and z components of a vector about the y Vector3f
     *
     * @param vec           vector to rotate
     * @param angle         angle in radians to rotate about the y Vector3f
